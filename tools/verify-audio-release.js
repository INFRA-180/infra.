...
function verifyRuntimeWiring(version) {
  ...
  assert(audioPrefetch.includes('CACHE_NAME: "infra-next-track-segments-v4"'), "startup segment audio cache is missing");
  assert(audioPrefetch.includes("STARTUP_BYTES: 2 * 1024 * 1024"), "startup segment byte budget is missing");
  assert(audioPrefetch.includes("QUEUE_DEPTH: 4"), "radio audio prefetch queue depth is missing");
  assert(audioPrefetch.includes('headers.set("Range"'), "audio prefetch must request a startup byte range");
  assert(audioPrefetch.includes("normalizeAudioResponseForCache"), "startup segments must be normalized for Cache Storage");
  assert(audioRadio.includes('if (reason === "playing") return true;'), "N+1 warmup must start on real playback");
  assert(serviceWorker.includes('const NEXT_TRACK_CACHE = "infra-next-track-segments-v4"'), "service worker must use the startup segment cache");
  assert(serviceWorker.includes("event.respondWith(servePrefetchedAudioOrNetwork(request, url))"), "R2 audio must use service-worker N+1 cache replay");
  assert(!serviceWorker.includes("Let Safari/PWA talk directly to R2"), "R2 audio must not bypass the N+1 service-worker path");
  assert(serviceWorker.includes("key === COMPLETE_NEXT_TRACK_CACHE"), "previous complete-track cache must be purged during service-worker activation");
  assert(serviceWorker.includes('cached.headers.get("X-Infra-Audio-Partial")'), "service worker must understand startup segment cache entries");
  ...
  assert(audioCore.includes("const isFastSkip = isAutoAdvance || isFromMediaSession || hasPreparedTransportTarget"), "transport fast path must not run without a prepared N+1 target");
  assert(audioCore.includes("isFromTransportControl && !hasPreparedTransportTarget"), "incomplete transport N+1 must be cancelled before fallback playback");
  assert(audioCore.includes('logAudioAuditEvent("ready_wait_start"'), "non-fast fallback playback must use the bounded readiness guard");
  assert(audioRadio.includes('strategy: "startup_segment"'), "N+1 telemetry must identify startup segment readiness");
  assert(audioRadio.includes("peekNextIndicesForPrefetch"), "radio prefetch must prepare a short queue");
  assert(audioRadio.includes("nextPrefetchReadySrcs"), "runtime must track multiple prepared startup segments");
  assert(!audioRadio.includes("function ensureNextPrefetchAudio()"), "N+1 must not depend on a hidden media preloader");
  ...
  assert(spaController.includes("const tracks = (isHome || needsAudio) ? loadTracksData() : Promise.resolve();"), "home must start loading tracks before the first tap");
  assert(spaController.includes("if (!isHome && !document.body.classList.contains(\"album-screen\")) cleanupIdleAudioContext"), "home init must not destructively reset audio");
  assert(spaController.includes('container.insertBefore(albums, directLinks || null)'), "home module ordering must keep albums before SEO direct links");
  ...
  assert(audioRadio.includes("duration,") && audioRadio.includes("seconds"), "radio playlist entries must preserve catalog durations");
  ...
  assert(spaController.includes('if (isHome)') && spaController.includes('scheduleInitialGlobalRandomPreparation("home_idle");'), "home PWA must prepare initial playback before the first tap");
  assert(read("public/assets/js/now-playing.js").includes("getNowPlayingCatalogDurationSeconds"), "fullscreen progress must use catalog duration before metadata");
  assert(css.includes(".now-playing-cover-wrap") && css.includes("background: #050609"), "fullscreen cover must hide transparent WebKit edge artifacts");
