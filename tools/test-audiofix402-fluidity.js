...
assert.match(restoreSource, /retainedHome\.setAttribute\("data-spa-route-state", "current"\)/);
assert.match(restoreSource, /resumeLiveHomeRoute\(\);\s*window\.scrollTo\(requested\.x, requested\.y\);/);
assert.match(restoreSource, /anchorCorrection = Math\.round\(anchor\.getBoundingClientRect\(\)\.top - route\.anchorViewportTop\)/);
assert.match(restoreSource, /outgoing\.remove\(\)/);
...
assert.match(spaRenderer, /routeHost\.appendChild\(destinationRoute\)/);
assert.match(spaRenderer, /return finish\("atomic_swap"\)/);
assert.match(spaRenderer, /handoff_strategy: mode === "view_transition" \? "native_view_transition" : "connected_route_stack"/);
assert.match(spaRenderer, /connected_home_retained:/);
assert.match(css, /data-spa-route-state="retained"/);
