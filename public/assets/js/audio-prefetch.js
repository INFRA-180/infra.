(function () {
  ...
  const constants = Object.freeze({
    ENABLED: true,
    CACHE_NAME: "infra-next-track-segments-v4",
    MAX_BYTES: 15 * 1024 * 1024,
    THRESHOLD_SECONDS: 30,
    STARTUP_BYTES: 2 * 1024 * 1024,
    QUEUE_DEPTH: 4,
    CONCURRENCY: 2,
    MAX_ENTRIES: 6
  });
  ...
  function getTotalLength(response) {
    if (!response || !response.headers) return 0;
    const contentRange = response.headers.get("Content-Range") || response.headers.get("content-range") || "";
    const rangeMatch = contentRange.match(/bytes\s+\d+-\d+\/(\d+)/i);
    if (rangeMatch) {
  ...
  function pruneCache(cache, maxEntries) {
    ...
    return cache.keys().then(function (keys) {
      ...
      return Promise.all(keys.slice(0, excess).map(function (key) { return cache.delete(key); })).then(function () {
        return true;
      });
    });
  }
  ...
  function putSingle(src, response, options) {
    ...
    return openCache().then(function (cache) {
      ...
      return normalizeAudioResponseForCache(response).then(function (cacheResponse) {
        ...
        return pruneCache(cache, opts.maxEntries || constants.MAX_ENTRIES);
      }).then(function () {
        return true;
      });
    });
