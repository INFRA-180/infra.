...
const release = "audiofix363-20260720";
const shellRelease = "infra-shell-20260720-audio363";
const cssRelease = "audiofix363-20260720";
...
expect(scripts.includes(`window.INFRA_BUILD_TAG = "${release}"`), "runtime build tag is not audiofix363");
expect(scripts.includes(`const runtimeVersion = "${release}"`), "runtime query version is not audiofix363");
expect(sw.includes(`const VERSION = "${shellRelease}"`), "Service Worker cache version is not audio363");
...
expect(visualizer.includes("size.height * 0.46"), "desktop frequency spectrum does not use the amplified mirrored range");
...
if (!process.exitCode) console.log("Audio stability checks passed for audiofix363.");
