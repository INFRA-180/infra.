...
const sandbox = {
  ...
  caches: {
    ...
    keys: () => Promise.resolve([
      ...
      "infra-shell-20260822-audio399-shell",
      "infra-shell-20260822-audio399-runtime"
    ]),
    delete: (name) => {
...
async function dispatchAudioFetch(headers, clientId, mode) {
  ...
  const request = {
    url: "https://infra180-api.pages.dev/audio/assets/music/streams/vtest/test/test.m4a",
    method: "GET",
    mode: mode || "cors",
    destination: "audio",
...
(async function () {
  ...
  assert(installedShellAssets.includes("./assets/js/scripts.js?v=audiofix399-20260822"));
  assert(
  ...
  assert(!deletedCaches.includes("infra-shell-20260822-audio399-shell"));
  assert(!deletedCaches.includes("infra-shell-20260822-audio399-runtime"));
  ...
  assert.strictEqual(response.headers.get("X-Infra-SW-Version"), "infra-shell-20260822-audio399");
