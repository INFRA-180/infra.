...
function verifyRuntimeWiring(version) {
  ...
  assert(audioPrefetch.includes('CACHE_NAME: "infra-next-track-full-v3"'), "complete N+1 audio cache is missing");
  assert(!audioPrefetch.includes("WARMUP_BYTES"), "N+1 must not use partial startup warmup");
  assert(audioPrefetch.includes("cache.put(request, response.clone())"), "complete N+1 responses must be stored directly");
  assert(audioRadio.includes('if (reason === "playing") return true;'), "N+1 warmup must start on real playback");
  assert(serviceWorker.includes('const NEXT_TRACK_CACHE = "infra-next-track-full-v3"'), "service worker must use the complete N+1 cache");
  ...
  assert(serviceWorker.includes("key === PARTIAL_NEXT_TRACK_CACHE"), "partial warmup cache must be purged during service-worker activation");
  ...
  assert(audioCore.includes("const isFastSkip = isAutoAdvance || isFromMediaSession || hasPreparedTransportTarget"), "transport fast path must not run without a completed N+1 target");
  ...
  assert(audioRadio.includes('strategy: "full_cache"'), "N+1 telemetry must identify complete-cache readiness");
