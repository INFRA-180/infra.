window.INFRA_BUILD_TAG = "audiofix407-20260825";
try {
...
(function () {
  ...
  const SPA_SHELL_VERSION = "infra-shell-20260825-audio407";
  ...
  const SPA_SCROLL_HISTORY_DEBOUNCE_MS = Number.isFinite(Number(spaRouterConstants.SCROLL_HISTORY_DEBOUNCE_MS))
    ...
    : 240;
  // Tie the live snapshot to the installed shell. A previous release may be
  // structurally valid while still pointing at a cover URL removed by the
  // current deployment; reusing that cache would overwrite the fresh Home.
  const LIVE_CATALOG_CACHE_NAME = `${SPA_SHELL_VERSION}-catalog`;
  const LIVE_CATALOG_TIMEOUT_MS = 3500;
  const LOCAL_CATALOG_VERSION = "audiofix407-20260825";
  ...
  const runtimeVersion = "audiofix407-20260825";
  ...
  function getAlbumCoverWarmupUrls(tracksData) {
    ...
    albums.forEach(function (album) {
      ...
      const canonical = normalizeCoverUrl(toRuntimeAbsoluteUrl(raw), { width: 1200 });
  ...
  function primeLinkedAlbumCoverForPwa(link, targetUrl) {
    ...
    preloadImage(url, { highPriority: true }).then(function (result) {
      ...
      // Only block duplicate requests while the current decode is running.
      // A transient mobile/network failure must remain retryable on the next tap.
      audioState.albumCoverPrimeUrls.delete(url);
    });
    return url;
  }
  ...
  function getCurrentAlbumTitle() {
    ...
    if (!routeRoot) return "";
    const heading =
      routeRoot.querySelector("main.page > h1") ||
      routeRoot.querySelector("main > h1") ||
      routeRoot.querySelector(".album-hero h1");
  ...
  function bindMediaSessionActions(options) {
    ...
    if (iosStandalone) {
      // Keep direct timeline scrubbing through seekto, but explicitly remove
      // the +/- 10-second actions so WebKit exposes playlist Previous/Next.
