...
async function main() {
  ...
  assert.ok(strokeCount >= 2, "pure live frequency-spectrum lines were not drawn");
  ...
  assert.equal(clipCount, 0, "pure FFT unexpectedly uses a particle clip");
  assert.equal(powderCanvasCount, 0, "pure FFT created a powder surface");
  assert.equal(powderImageWriteCount, 0, "pure FFT rasterized a powder frame");
  assert.equal(powderNonzeroPixelCount, 0, "pure FFT contains powder pixels");
  assert.equal(powderUpperPixelCount, 0, "pure FFT contains upper powder pixels");
  assert.equal(powderLowerPixelCount, 0, "pure FFT contains lower powder pixels");
  assert.equal(drawImageCount, 0, "pure FFT composited a powder buffer");
  ...
  assert.equal(powderCanvasCount, 0, "animation created a powder surface in pure FFT mode");
  analyserMode = "silence";
  audio.paused = true;
  mediaListeners.get("pause")();
  assert.ok(cancelledFrames > 0, "pause did not cancel the animation frame");
  assert.equal(requestedAnimationFrame, null, "pure FFT keeps animating after playback pauses");
  ...
  assert.equal(health.visualizer_powder_particle_count, 0, "health report exposes powder in pure FFT mode");
  assert.equal(health.visualizer_powder_earth_particle_count, 0, "health report exposes Earth powder");
  assert.equal(health.visualizer_powder_moon_particle_count, 0, "health report exposes lunar powder");
  assert.equal(health.visualizer_powder_upper_particle_count, 0, "health report exposes upper powder");
  assert.equal(health.visualizer_powder_lower_particle_count, 0, "health report exposes lower powder");
  assert.equal(health.visualizer_powder_helix_particle_count, 0, "health report exposes a helix");
  assert.equal(health.visualizer_powder_helix_active_count, 0, "health report sees a helix still moving after settling");
  assert.equal(health.visualizer_powder_helix_max_active_count, 0, "health report exposes active helix grains");
  assert.equal(health.visualizer_powder_helix_max_offset_px, 0, "health report exposes a helix radius");
  assert.equal(health.visualizer_powder_surface_ready, 0, "health report sees a powder surface");
  assert.equal(health.visualizer_powder_kick_count, 0, "health report contains a particle kick");
  assert.equal(health.visualizer_powder_max_airborne_count, 0, "health report contains an airborne particle");
  assert.equal(health.visualizer_powder_max_rise_px, 0, "health report exposes a ballistic rise");
  assert.equal(health.visualizer_powder_airborne_count, 0, "health report sees particles still airborne after settling");
  assert.equal(health.visualizer_powder_gravity_milli, 0, "health report exposes Earth gravity");
  assert.equal(health.visualizer_powder_moon_gravity_milli, 0, "health report exposes lunar gravity");
